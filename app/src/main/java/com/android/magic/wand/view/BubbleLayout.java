...
public class BubbleLayout extends View {
    ...
    protected void onDraw(Canvas canvas) {
            ...
            new Thread() {
                public void run() {
                    while (true) {
                        ...
                        int radius = random.nextInt(80);
                        while (radius == 0) {
                            radius = random.nextInt(80);
                        }
        ...
        //绘制气泡
        paint.reset();
        paint.setColor(getResources().getColor(android.R.color.holo_red_light));//灰白色
        paint.setAlpha(120);//设置不透明度：透明为0，完全不透明为255
